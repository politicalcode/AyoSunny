...
import com.example.views.SBViewUtils;

public class Layout_Dp_Calculator extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_layout__dp__calculator);
		self = this;
		...
		btn_add_id.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				try {
					...
					int id = getResources().getIdentifier(ss[2], ss[1], "com.example.views");
		...
		btn1.setOnClickListener(new View.OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				DrawableScannerAndPicker.lookDrawable(self, getApplicationContext().getPackageName(), "drawable");
			}
		});
		...
		btn2.setOnClickListener(new View.OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				DrawableScannerAndPicker.lookDrawable(self, "android", "drawable");
			}
		});
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == DrawableScannerAndPicker.REQEUST_CODE){
	...
	private void tellHim(){
		...
		tv_dpw = SBViewUtils.getDp(self, tv_w);
		tv_dph = SBViewUtils.getDp(self, tv_h);
		
		int dpi = SBViewUtils.getDpi(self);
		
		StringBuilder sb = new StringBuilder();
		sb.append("在您的手机（" + SBViewUtils.getDpiType(self) + "）上：\n----显示的大小是：");
