...
/**
 * 一、发现奇怪的问题？ 在研究Android Drawable资源的时候，发现了一个奇怪的问题。在官方API介绍中： ShapeDrawable
 * 介绍：This object can be defined in an XML file with the <shape>
 * element（这个对象可以用<shape>元素在xml文件中定义） GradientDrawable 介绍：This object can be
 * defined in an XML file with the <shape> element（这个对象可以用<shape>元素在xml文件中定义）
 * 两者的介绍一模一样，都说可以使用<shape>标签在xml文件中定义。 那么，到底用<shape>标签定义的是什么的呢？经过下面的实验： TextView
 * textView=(TextView)findViewById(R.id.textView); ShapeDrawable
 * gradientDrawable=(ShapeDrawable)textView.getBackground();
 ...
 * 
 */
public class D_GradientDrawable extends Activity {
...
//	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.activity_d__gradient_drawable, menu);
		return true;
	}
