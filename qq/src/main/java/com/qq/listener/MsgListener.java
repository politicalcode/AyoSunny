...
public class MsgListener implements MessageListener{
	...
	public void processMessage(Chat arg0, Message message) {
		try {
			...
			String to = message.getTo(); //msgs[0];//接收者,当然是自己
			String from = message.getFrom() + ""; //msgs[1];//发送者，谁给你发的消息
			String msgtype = message.getType().name(); //msgs[2];//消息类型
			String msgcontent = message.getBody(); //msgs[3];//消息内容
			String msgtime= "0000"; // msgs[4];//消息时间
